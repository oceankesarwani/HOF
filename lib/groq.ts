...
Return ONLY a JSON object in this format:
{
  ...
  try {
    return JSON.parse(resText) as PRHealthResult;
  } catch (e) {
    return {
      score: 50,
...
export async function detectAIGeneratedPR(
  ...
  // Using a simple regex to find likely function calls/names in the diff
...
Return ONLY a JSON object:
{
  ...
  const resText = completion.choices[0]?.message?.content ?? "{}";
  try {
    return JSON.parse(resText) as AIDetectionResult;
  } catch (e) {
    return {
      score: 0,
      confidence: "Low",
